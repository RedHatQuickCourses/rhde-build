...
1. Install the packages for the Image Buider service and its clients on your development machine.
...
.. Enable and activiate the Cockpit service, if not already enabled on your development machine.
+
[source,subs="verbatim,quotes"]
--
...
.. You cannot configure repositoy overrides for RHEL packages using the Image Builder web UI but you can verify that the changes are in effect.
+
