...
.. Build a new iso [ really need -no-emul-boot twice? ]
+
[source,subs="verbatim,quotes"]
--
...
+
If I can make customizations.installer to work with newer RHEL:
+
[source,subs="verbatim,quotes"]
--
...
--
+
Else, if have to hack the kickstart inside the iso: [ NOT WORKING --boot uefi made no difference ]
+
[source,subs="verbatim,quotes"]
--
$ *virt-install --name edge-test-2 --os-variant rhel9.2 \
--memory 4096 --vcpus 2 --disk size=40 --graphics=none \
--location /home/student/new-installer.iso
--extra-arg console=ttyS0 -v*
--
+
.. Gambiarra, getting ks from network [works... what I did wrong on iso hack? ]
+
[source,subs="verbatim,quotes"]
--
virt-install --name edge-db-1 --os-variant rhel9.2  --memory 4096 --vcpus 2 --disk size=40 --location /home/student/new-installer.iso --graphics=none --extra-arg console=ttyS0 -v --extra-arg inst.ks=http://servera.lab.example.com/rhel9-mysql.ks
--
+
