...
.. Ensure that your bootable ISO is compatible with UEFI-based systems.
+
[source,subs="verbatim,quotes"]
--
...
.. Mount the new ISO in a temporary directory and check that it contains the customized kickstart file, then unount it.
+
[source,subs="verbatim,quotes"]
--
...
+
* Use a regular grapphical boot, from a virtual CD-ROM reader.
+
...
--
+
* Use a virtual serial console, and reference the kickstart in the ISO image.
+
...
+
WARNING: Do not delete (undefine) your _database VM_, you will use it during the next chapter to perform system updates.
...
.. If you had issues with booting or acessing your _database VM_, please review the xref:s2-boot-lab.adoc[previous lab] for Libvirt troubleshooting hints using the `virsh` command. It may also help trying a different VM creation method, for example, if you used `--location`, try using `--cdrom`.
