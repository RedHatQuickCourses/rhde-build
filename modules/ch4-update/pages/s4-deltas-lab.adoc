...
.. Increment the version number, in tbe beginning of the TOML file, and add a `packages` section which refer to packages from Nano.
+
[source,subs="verbatim,quotes"]
--
...
.. Configure you _database VM_ with an OSTree remote that points to your _web server_ machine.
+
[source,subs="verbatim,quotes"]
--
...
.. Check that your _database VM_ can find available dates on the remote OSTree repository.
+
[source,subs="verbatim,quotes,attributes"]
--
...
7. Still on your _development machine_, verify that your _database VM_ actually fetched upgrades using static deltas.

.. Search the system logs on the _database VM_. You should see two entries with a message `libostree pull`: one from checking for updates, which referes to no deltas and has a small transfer size, and another from the actual upgrade operation, which refers to a delta and with a larger transfer size.
+
[source,subs="verbatim,quotes"]
--
...
.. If you seach the system log of the _test VM_ from the previous labs, your would see entries similar to the folowing.
+
[source,subs="verbatim,quotes"]
--
[core@edge ~]$ sudo journalctl --no-pager -t rpm-ostree -g pull
[sudo] password for core: 
-- Boot f744fcc757d74cb9823d3315a3a5d373
...
<.> From now on, many requests, one for each individual file and directory in the OSTree commit.
...
This was the last activity in this course. Stay tuned for the reminaing Red Hat Device Edge courses, which feature MicroShift, Ansible Automation, Green Boot and FDO.