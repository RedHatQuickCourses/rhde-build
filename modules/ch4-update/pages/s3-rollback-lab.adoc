...
.. Increment the version number, in tbe beginning of the TOML file, and add another `packages` section. There is no need to change any of the `customizations` section. After your edits, the TOML file should look like the following:
+
[source,subs="verbatim"]
--
include::1@samples:blueprints:example$rhel9-httpd-v3.toml[lines=1..15]
...
5. Back to your _development machine_, stage the new edge commit image on your _test VM_.
...
7. Still on your _developer machine_, suppose you discovered the new edge commit image contains a serious security issue and you must rollback your _test VM_ to the previous system image, to verify that the bad update can be rolled back.
...
.. After reboot (and relogin) both commits are still in the system, but they switch order. Notice that the second deployment is the newer commit, which was the bad update, and is now inactive.
+
[source,subs="verbatim,quotes,attributes"]
--
...
As you see, rolling back an RPM-OSTree system just switches which deployment is active for the next boot. The newer but inactive system image remains available in edge devices, and could me marked active again, until a future upgrade removes it.
